...
Медик. Покупает и продаёт медикаменты (`med`), умеет лечить (`canHeal: true`, базовая цена **50 💎**).

| Ур. | Бонус |
|---|---|
| каждый уровень | **+10% к максимальному HP** ([`getMaxHp()`](src/js/core/utils.js)) |
| каждый уровень | **−10% к стоимости лечения** ([`buyHeal()`](src/js/logic/trade.js)) |
| 10 | Лечение **бесплатно** |

Стоимость лечения также снижается до **35 💎** при экипированном Торговом чипе `eq_trade`.
...
| `npc_med` | Доктор Кроу | med | med | Лечение 50 💎, +10% макс. HP и −10% к лечению за уровень |
