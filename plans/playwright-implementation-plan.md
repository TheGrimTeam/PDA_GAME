...
- [x] Папка `tests/` создана и изолирована от корня проекта.
- [x] `package.json`, `tsconfig.json`, `playwright.config.ts`, `.gitignore` созданы.
- [x] Зависимости и браузеры (Chromium, Firefox, WebKit) установлены.
- [x] `webServer` автоматически поднимает локальный сервер.
- [x] Моки камеры, аудио и `Math.random` работают.
- [x] Фикстуры `game`, `player`, `two-players` доступны.
- [x] Smoke-тест загрузки приложения проходит во всех трёх браузерах.
- [x] `index.html` актуален относительно `src/`.

**Статус: внедрение завершено.** Smoke-набор (`tests/e2e/smoke/`) проходит
12/12 (4 теста × 3 браузера) за ~4.5 с. Запуск: `cd tests && npm test`.
